...
\usepackage{./stylings/es6listings}
\usepackage{./stylings/weblistings}
\renewcommand{\lstlistlistingname}{Spis listingów}
\renewcommand{\figurename}{Rys.}

\usepackage[titles]{tocloft}
\renewcommand{\cftfigpresnum}{Rys. } % put before the number
...
\AtBeginEnvironment{figure,lstlisting}{\setlength{\intextsep}{1.6\baselineskip}}
...
\chapter{Wprowadzenie do mikroserwisów}
  ...
  Taką strukturę trójwarstwową możemy opisać rysunkiem \ref{fig:monolith_diagram}
  ...
  Zaczęto więc do większych aplikacji stosować podział aplikacji na części będące usługami niezależnymi od pozostałych elementów systemu. Taką pojedynczą usługę nazywamy mikroserwisem. Przykładowe rozmieszczenie modułów w projekcie mikroserwisowym możemy zaobserwować na rysunku \ref{fig:microservices_diagram}
...
\chapter{Mikrofrontendy}
  ...
  Dla każdego z nich istnieje zestaw zastosowań biznesowych dla których rozwiązanie nie będzie właściwe, ale będą istnieć też takie, dla których będą one właściwe. Każde z nich zostanie omówione dokładniej omówione w poniższych podsekcjach. 
    ...
    \lstinputlisting[style=htmlcssjs, label=lst:iframe_microfrontends, caption=Przykładowa implementacja mikrofrontendu za pomocą \lstinline{<iframe>} (przykład pochodzi z \cite{fowler_2019})]{listings/iframe_microfrontends.html}

    \subsection{Przechowywanie mikrofrontendów w skryptach JavaScript}
    Idea ładowania mikrofrontendów przez podpinanie skryptów JavaScript jest bardzo prosta. Zgodnie z przykładem implementacji podanym na listingu \ref{lst:js_microfrontends}, na początku sekcji \lstinline{<body>} podpiete są trzy skrypty reprezentujące trzy mikrofrontendy. Obecny jest także element \lstinline{<div id="micro-frontend-root">}, do którego będą ładowane mikrofrontendy. W skrypcie podanym niżej znajduje się przyporządkowanie tras w routingu do mikrofrontendów o odpowiednich nazwach oraz referencja na element \lstinline{<div id="micro-frontend-root">}

    \lstinputlisting[style=htmlcssjs, label=lst:js_microfrontends, caption=Przykładowa implementacja mikrofrontendu za pomocą skrpytów JavaScript (przykład pochodzi z \cite{fowler_2019})]{listings/microfrontends.html}

    \subsection{Dynamiczne ładowanie modułów poprzez menedżer pakietów}
    Istnieje również grupa rozwiązań oparta na dynamicznym ładowaniu modułów. Do grupy rozwiązań tego typu należą wspomniane na wykresie procentowym z rysunku \ref{fig:tsh_frontend_implementations} Module Federation, Bit oraz Systemjs. Pierwsze z rozwiązań, Module Federation, jest wtyczką do transpilatora Webpack. Wszystkie trzy, pomimo pewnych niewielkich różnic w zakresie składni poleceń, instalacji i procesu konfiguracji, działają one bardzo podobnie. 
